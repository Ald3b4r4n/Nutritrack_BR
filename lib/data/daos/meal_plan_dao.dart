...
class MealPlanDao extends DatabaseAccessor<AppDatabase> with _$MealPlanDaoMixin {
  ...
  Future<domain.MealPlan> upsertMealPlan(domain.MealPlan plan) async {
    await into(mealPlans).insertOnConflictUpdate(MealPlansCompanion(
      id: Value(plan.id),
  ...
  Future<domain.MealPlan?> getActiveMealPlan() async {
    final row = await (select(mealPlans)..where((t) => t.isActive.equals(true))).getSingleOrNull();
  ...
  Future<List<domain.MealPlan>> getAllMealPlans() async {
    ...
    return rows
        .map((r) => domain.MealPlan(id: r.id, name: r.name, isActive: r.isActive))
        .toList();
  }

  // --- Meal Plan Days ---

  Future<List<domain.MealPlanDay>> upsertMealPlanDays(List<domain.MealPlanDay> days) async {
    await batch((batch) {
      batch.insertAllOnConflictUpdate(
        mealPlanDays,
        days.map((d) => MealPlanDaysCompanion(
              id: Value(d.id),
  ...
  Future<List<domain.MealPlanDay>> getMealPlanDays(String mealPlanId) async {
    final rows = await (select(mealPlanDays)..where((t) => t.mealPlanId.equals(mealPlanId))).get();
    return rows
        .map((r) => domain.MealPlanDay(
              id: r.id,
  ...
  Future<List<domain.MealPlanEntry>> upsertMealPlanEntries(List<domain.MealPlanEntry> entries) async {
    await batch((batch) {
      batch.insertAllOnConflictUpdate(
        mealPlanEntries,
        entries.map((e) => MealPlanEntriesCompanion(
              id: Value(e.id),
  ...
  Future<List<domain.MealPlanEntry>> getMealPlanEntries(String mealPlanDayId) async {
    final rows = await (select(mealPlanEntries)..where((t) => t.mealPlanDayId.equals(mealPlanDayId))).get();
    return rows
        .map((r) => domain.MealPlanEntry(
              id: r.id,
